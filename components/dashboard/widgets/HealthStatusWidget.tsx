...
function daysRemaining(since: string | null, estimatedDays: number | null): number | null {
  ...
  const diff = Math.ceil((recovery.getTime() - today.getTime()) / (1000 * 60 * 60 * 24))
  return diff
}

export function HealthStatusWidget({ status }: { status: HealthStatus | null }) {
  if (!status || (!status.sick && !status.injured)) return null

  const sickDaysLeft = daysRemaining(status.sick_since, status.sick_estimated_days)
  const injuryDaysLeft = daysRemaining(status.injury_since, status.injury_estimated_days)
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="text-sm font-medium text-text">
                {status.injury_description ?? 'Injured'}
              </p>
              {injuryDaysLeft != null && injuryDaysLeft > 0 && (
                <p className="text-xs text-text-muted">~{injuryDaysLeft} day{injuryDaysLeft !== 1 ? 's' : ''} to recovery</p>
              )}
              {injuryDaysLeft != null && injuryDaysLeft <= 0 && (
                <p className="text-xs text-accent">Should be recovered — log an update</p>
              )}
              {status.injury_since && (
                <p className="text-xs text-text-subtle">
                  Since {new Date(status.injury_since).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}
                </p>
              )}
            </div>
          </div>
